...
export class AwsEc2IsaaclabSoarm101GuiStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // Note: CPU-based auto-stop (CloudWatch Alarm on CPUUtilization < 2%) was removed.
    // While Isaac Sim is running, CPU stays ~70% even when idle, so a CPU threshold can't
    // distinguish "in use" from "left running". Idle/forgotten-shutdown protection is better
    // handled out of band (periodic notify + manual stop, or a time-based scheduler) — see the
    // separate article. Day-to-day cost control here relies on the t3<->g5 swap and teardown.
    ...
    new cdk.CfnOutput(this, "SsmStartCommand", {
      ...
      description:
        "Interactive shell via SSM Session Manager (no SSH key, no inbound port)",
    });
    new cdk.CfnOutput(this, "DcvPortForwardCommand", {
      ...
      description:
        "Port-forward localhost:8443 -> EC2:8443 for Amazon DCV browser access",
    });
  }
}
