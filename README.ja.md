...
AWS EC2 + Amazon DCV で Isaac Sim / Isaac Lab の GUI を起動し、SO-ARM101 を手動操作するためのインフラ一式（AWS CDK / TypeScript）。**SSH キー不要・インバウンドポート全閉、SSM Session Manager + ポートフォワーディングで接続**します。
...
- **AWS Session Manager Plugin**（[インストール手順](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager-working-with-install-plugin.html)）
- Node.js 20 以上
- CDK が初回利用なら `cdk bootstrap` 実行可能なこと

> 本スタックは SSH キーペア / 許可 CIDR を要求しません。接続は SSM Session Manager のみです。
...
デプロイ完了後、Outputs には以下が表示されます:

- `InstanceId` — stop/start で不変
- `SsmStartCommand` — SSM Session Manager 対話シェル
- `DcvPortForwardCommand` — `localhost:8443` → `EC2:8443` のポートフォワーディング

## 4. 動作確認（SSM 接続）

`scripts/connect.sh` が InstanceId を Name タグから引いて SSM セッションを開きます:
...
| スクリプト | 実行場所 | 役割 |
|--|--|--|
| `scripts/connect.sh` | ローカル PC | Name タグから InstanceId を引いて SSM 対話シェル接続 |
| `scripts/dcv-port-forward.sh` | ローカル PC | `localhost:8443` → `EC2:8443` の SSM ポートフォワーディング |
...
2. `./scripts/connect.sh` で SSM 接続
3. EC2 上で `./setup-docker.sh` 実行 → SSM セッションを抜けて再接続
4. ローカルから `./scripts/switch-to-g5.sh` で g5 切替
5. `./scripts/connect.sh` で SSM 再接続（g5）
6. EC2 上で `./setup-dcv.sh` → 終了後 `sudo reboot`
7. **別ターミナル**で `./scripts/dcv-port-forward.sh` を起動（ポートフォワーディング）
8. ブラウザで `https://localhost:8443`（DCV 接続）
9. DCV デスクトップのターミナルで `./setup-isaac.sh`
10. 同じ DCV ターミナルで `./scripts/launch-isaac.sh` で Isaac Sim Native GUI を 1 行起動 → URDF Import → Physics Inspector で操作
11. 終わったらローカルから `./scripts/switch-to-t3.sh` で t3 戻し
12. 完了時 `./scripts/teardown.sh` で完全削除
